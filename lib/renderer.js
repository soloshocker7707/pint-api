...
/**
 * Advanced Rendering Engine for Pint API
...
export class Renderer {
    ...
    await this.page.evaluate((selectors) => {
      ...
      all.forEach(el => {
        ...
        if (style.position === 'fixed' || style.position === 'sticky') {
          ...
          // If element covers more than 50% of viewport or is a known popup pattern
          if (area > viewportArea * 0.5 || parseInt(style.zIndex) > 1000) {
            el.style.display = 'none';
          }
        }
      });
  ...
  async smartWait() {
    ...
    // 3. Wait for stability (uses our existing lib/smart-wait.js)
